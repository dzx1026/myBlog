...
def auth_factory(app, handler):
    ...
    def auth(request):
        ...
        if cookie_str:
            user = yield from handlers.cookie2user(cookie_str)
            if user:
...
def logger_factory(app, handler):
    ...
    def logger(request):
        ...
        return (yield from handler(request))  # 为什么response没有返回web.Response，handler就成了none？？？
...
def datetime_filter(t):
    ...
    if delta < 60:
        return u'1分钟前'  # u表示将后面跟的字符串以unicode格式存储
    if delta < 3600:
...
@asyncio.coroutine
def init(loop):
    yield from myorm.create_pool(loop=loop, host='127.0.0.1', port=3306, user='dzx', passwd='123456', db='myblog')
